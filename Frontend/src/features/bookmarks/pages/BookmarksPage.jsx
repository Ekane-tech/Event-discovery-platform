    ...
    <PageContainer>
      <section className="overflow-hidden rounded-3xl bg-linear-to-r from-yellow-500 to-orange-600 p-8 text-white shadow-sm">
        <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
          <div>
            <span className="inline-flex items-center gap-2 rounded-full bg-white/15 px-4 py-2 text-sm font-bold backdrop-blur"><Bookmark className="h-4 w-4" /> {t('bookmarks.savedEvents')}</span>
            <h1 className="mt-5 text-4xl font-black">{t('bookmarks.yourBookmarkedEvents')}</h1>
            <p className="mt-3 max-w-2xl text-white/90">{t('bookmarks.subtitle')}</p>
          </div>
        ...
        <div className="mb-6 max-w-sm">
          {loading ? <StatCardSkeleton compact /> : (
            <div className="relative overflow-hidden rounded-3xl bg-linear-to-br from-yellow-500 to-orange-600 p-5 text-white shadow-sm">
              ...
              <p className="relative mt-4 text-sm font-bold text-white/90">{t('bookmarks.savedEvents')}</p>
            </div>
          )}
        </div>
        {loading && <EventGridSkeleton count={6} />}
        {error && <ErrorState title={t('bookmarks.loadError')} message={error} />}
        {!loading && !error && bookmarkCount === 0 && <EmptyState title={t('bookmarks.emptyTitle')} message={t('bookmarks.emptyMessage')} />}
