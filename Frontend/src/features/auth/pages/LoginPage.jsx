...
const quickAccess = [
  { title: 'Attendee', description: 'Discover events, register, save tickets.', icon: Ticket, email: 'user@example.com' },
  { title: 'Organizer', description: 'Create events and manage attendees.', icon: CalendarCheck, email: 'organizer@example.com' },
]
    ...
    <main className="mx-auto grid min-h-[calc(100vh-220px)] max-w-7xl gap-10 px-4 py-10 sm:px-6 lg:grid-cols-[1fr_480px] lg:px-8 lg:py-14">
      <section className="relative hidden overflow-hidden rounded-[2rem] bg-slate-950 bg-cover bg-center p-10 text-white lg:block" style={{ backgroundImage: 'linear-gradient(140deg, rgba(2,6,23,.82), rgba(15,118,110,.6)), url(/hero-events.svg)' }}>
        <div className="relative z-10 flex h-full flex-col justify-between">
          <div>
            <span className="inline-flex rounded-full bg-white/10 px-4 py-2 text-sm font-bold text-teal-100">Welcome back</span>
            <h2 className="mt-6 max-w-xl text-5xl font-black leading-tight">Sign in and continue your event journey.</h2>
            <p className="mt-5 max-w-lg text-lg leading-8 text-slate-100">Access your dashboard, tickets, notifications, and event tools in one secure place.</p>
          </div>
          <div className="grid gap-4">
            {quickAccess.map((item) => {
              const Icon = item.icon
              return <button key={item.title} onClick={() => chooseAccount(item.email)} className="rounded-3xl bg-white/10 p-5 text-left backdrop-blur transition hover:bg-white/15"><div className="flex gap-4"><span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white/15"><Icon className="h-5 w-5" /></span><span><strong>{item.title}</strong><span className="mt-1 block text-sm text-slate-200">{item.description}</span></span></div></button>
            })}
          </div>
        </div>
      </section>
      ...
      <section className="flex items-center">
        <AuthCard eyebrow={t('auth.secureLogin')} title={t('auth.signIn')} description="Choose your account type or enter your credentials to continue." footer={<>No account yet? <Link className="font-bold text-teal-700" to="/register">Create an account</Link></>}>
          <div className="mb-5 grid gap-3 sm:grid-cols-2 lg:hidden">
            {quickAccess.map((item) => {
              const Icon = item.icon
              return <button key={item.title} onClick={() => chooseAccount(item.email)} className="rounded-2xl border border-slate-200 p-3 text-left hover:border-teal-300 hover:bg-teal-50"><Icon className="h-5 w-5 text-teal-700" /><p className="mt-2 font-bold">{item.title}</p><p className="text-xs text-slate-500">{item.description}</p></button>
            })}
          </div>
