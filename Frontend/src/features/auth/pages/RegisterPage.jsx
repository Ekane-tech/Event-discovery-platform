...
const ACCOUNT_TYPES = [
  { value: 'user', titleKey: 'auth.attendEvents', descriptionKey: 'auth.attendeeDescription', icon: Ticket },
  { value: 'organizer', titleKey: 'auth.provideServices', descriptionKey: 'auth.organizerDescription', icon: CalendarCheck },
]
      ...
      <section className="hidden lg:block">
        <div className="sticky top-28 overflow-hidden rounded-[2rem] bg-slate-950 shadow-2xl">
          ...
          <div className="absolute bottom-0 p-10 text-white">
            ...
            <h2 className="text-4xl font-black">{t('about.heading')}</h2>
            <p className="mt-4 max-w-lg text-slate-200">{t('about.description')}</p>
          </div>
        </div>
      </section>
        ...
        >
            ...
            {ACCOUNT_TYPES.map((item) => {
              ...
              return (
                <button
                  key={item.value}
                  type="button"
                  onClick={() => chooseAccountType(item.value)}
                  className={`rounded-2xl border p-4 text-left transition ${active ? 'border-teal-600 bg-teal-50 ring-2 ring-teal-100' : 'border-slate-200 hover:border-teal-200 hover:bg-slate-50'}`}
                >
                  <Icon className="h-6 w-6 text-teal-700" />
                  <h3 className="mt-3 font-bold text-slate-950">{t(item.titleKey)}</h3>
          ...
          {form.accountType && (
            <form onSubmit={handleSubmit} className="grid gap-4">
              <FormInput label={t('auth.fullName')} name="name" value={form.name} onChange={updateField} placeholder="Your name" required />
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <FormSelect label={t('auth.region')} name="region" value={form.region} onChange={updateField}>
                  {CAMEROON_REGIONS.map((region) => <option key={region}>{region}</option>)}
                </FormSelect>
                <FormSelect label={t('auth.preferredLanguage')} name="preferredLanguage" value={form.preferredLanguage} onChange={updateField}>
