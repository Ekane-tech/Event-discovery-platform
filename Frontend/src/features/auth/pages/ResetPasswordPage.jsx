    ...
    <main className="relative overflow-hidden px-4 py-12 sm:px-6 lg:px-8">
      ...
      <div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[.9fr_1.1fr] lg:items-center">
        <section className="rounded-[2rem] bg-gradient-to-br from-slate-950 to-teal-900 p-8 text-white shadow-2xl shadow-slate-200">
          ...
          <h2 className="mt-6 text-3xl font-black leading-tight">Create a new secure password.</h2>
          <p className="mt-3 text-sm leading-7 text-slate-200">
            Your reset token is handled privately from the link. For security, it is never displayed on this page.
          </p>
        ...
        <div className="mx-auto w-full max-w-xl">
          <AuthCard
            ...
            description="Choose a strong password that you do not use on other websites."
            footer={<Link className="inline-flex items-center gap-1 font-bold text-teal-700" to="/login"><ArrowLeft className="h-4 w-4" /> Return to login</Link>}
          >
            {success && (
            ...
            {!linkIsValid && (
              <div className="mb-5 rounded-2xl border border-amber-100 bg-amber-50 p-4 text-sm leading-6 text-amber-900">
                This password reset link is incomplete or invalid. Please request a new reset link from the forgot password page.
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="grid gap-4">
              ...
              <FormInput label="New password" name="password" type="password" value={form.password} onChange={updateField} required autoComplete="new-password" />
              <FormInput label="Confirm password" name="passwordConfirmation" type="password" value={form.passwordConfirmation} onChange={updateField} required autoComplete="new-password" />
