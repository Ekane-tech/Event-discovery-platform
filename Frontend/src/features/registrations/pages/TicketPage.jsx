        ...
        <div className="overflow-hidden rounded-[2rem] bg-white shadow-2xl ring-1 ring-slate-200">
          <div className="bg-slate-950 bg-cover bg-center p-5 text-white sm:p-6 md:p-8" style={{backgroundImage:`linear-gradient(90deg, rgba(2,6,23,.9), rgba(15,118,110,.7)), url(${event.coverImage?.url || '/hero-events.svg'})`}}>
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-sm font-bold text-blue-100"><ShieldCheck className="h-4 w-4"/> {t('tickets.digitalTicket', 'Digital Ticket')}</p>
              ...
              <div className="shrink-0 rounded-2xl bg-white p-3 text-slate-950 shadow-xl">
                <div className="mb-2 flex items-center justify-center gap-1.5 text-[10px] font-black text-blue-700 sm:text-xs"><img src="/applogo.png" alt={APP_NAME} className="h-6 w-6 rounded-lg object-cover sm:h-7 sm:w-7" /> {APP_NAME}</div>
          ...
          <div className="p-5 sm:p-6 md:p-8">
            ...
            <div className="grid grid-cols-2 gap-3 md:gap-5">
              ...
              <p className="col-span-2 flex gap-2 text-xs text-slate-700 md:col-span-1 md:text-sm"><MapPin className="h-4 w-4 shrink-0 text-blue-700 md:h-5 md:w-5" /> {event.venue}, {event.city}, {event.region}</p>
              <p className="col-span-2 flex gap-2 text-xs text-slate-700 md:col-span-1 md:text-sm"><QrCode className="h-4 w-4 shrink-0 text-blue-700 md:h-5 md:w-5" /> {t('tickets.scanHint', 'Scan this QR code to verify the ticket.')}</p>
            </div>
