    ...
    <PageContainer>
      ...
      <Modal open={modal.open} title={modal.category ? 'Edit category' : 'Create category'} onClose={closeModal}>
        <form onSubmit={handleSubmit} className="grid gap-2">
          <div className="rounded-3xl bg-gradient-to-br from-indigo-600 to-teal-700 p-3 text-white">
            <p className="text-sm font-bold uppercase tracking-wide text-white/80">Category setup</p>
            <h3 className="mt-1 text-2xl font-black">{modal.category ? 'Update category' : 'New category'}</h3>
            <p className="mt-2 text-sm leading-6 text-white/90">Use short, recognizable names that help attendees filter events quickly.</p>
          </div>
          ...
          <div className="rounded-3xl border border-dashed border-slate-300 bg-slate-50 p-2">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
              <div className="h-28 w-full overflow-hidden rounded-2xl bg-white sm:w-40">
                {form.image ? (
              ...
              <div className="flex-1">
                ...
                <p className="mt-1 text-sm text-slate-600">JPG, PNG or WebP. Maximum 4MB. This image appears on public category cards and as an event fallback image.</p>
                <div className="mt-3 flex flex-wrap gap-2">
                  <label className="inline-flex cursor-pointer items-center rounded-xl bg-teal-700 px-4 py-2 text-sm font-bold text-white transition hover:bg-teal-800">
                  ...
                  {(form.image || modal.category?.image_url) && (
                    <Button type="button" variant="secondary" onClick={() => setForm((current) => ({ ...current, image: null, removeImage: true }))}>
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button type="submit" disabled={saving}>{saving ? 'Saving...' : modal.category ? 'Update Category' : 'Create Category'}</Button>
          </div>
        </form>
      </Modal>
