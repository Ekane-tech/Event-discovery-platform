...
function RoleBadge({ role, t }) {
  const styles = { admin: 'bg-purple-100 text-purple-800', organizer: 'bg-blue-100 text-blue-800', user: 'bg-blue-100 text-blue-800' }
...
function UserMetric({ label, value, icon: Icon }) { return <MetricCard label={label} value={value} icon={Icon} /> }
...
export default function AdminUsersPage() {
  ...
  const rows = users.map((user) => {
    ...
    return {
      ...
      role: <div className="flex flex-wrap gap-1"><RoleBadge role={user.role?.name} t={t} />{user.role?.name === 'organizer' && user.profile?.is_verified_organizer && <span className="rounded-full bg-blue-50 px-2 py-1 text-xs font-black text-blue-700">{t('admin.common.verified', 'Verified')}</span>}</div>,
  ...
  return <PageContainer><AdminHero title={t('admin.users.title', 'Manage users')} description={t('admin.users.description', 'Review accounts, roles and access status across the platform.')} />
    <div className="mt-6 grid grid-cols-2 gap-4 xl:grid-cols-4"><UserMetric label={t('admin.users.totalUsers', 'Total users')} value={metrics.total} icon={Users} t={t}/><UserMetric label={t('admin.users.activeUsers', 'Active')} value={metrics.active} icon={UserCheck} t={t}/><UserMetric label={t('admin.users.suspendedUsers', 'Suspended')} value={metrics.suspended} icon={UserX} t={t}/><UserMetric label={t('admin.users.organizersCount', 'Organizers')} value={metrics.organizers} icon={ShieldCheck} t={t}/></div>
