...
export default function AdminDashboardPage() {
  ...
  return <PageContainer>
    <div className="mb-4 flex items-center justify-between">
      <h1 className="text-2xl font-black text-slate-950">System overview</h1>
      <Link to="/admin/events"><Button variant="light">Moderate Events</Button></Link>
    </div>
    <div className="grid grid-cols-2 gap-4 xl:grid-cols-4">{metrics.map(([label,value,Icon,gradient])=><AdminMetricCard key={label} label={label} value={value} icon={Icon} gradient={gradient}/>)}</div>
